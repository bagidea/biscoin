...
#if defined(NDEBUG)
# error "Biscoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0xfc24fbf3f5432b1e6abf74fc35a6f77bdfef0441bf74e9e4db2f968af5055b22");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Biscoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Biscoin Signed Message:\n";
...
{
    // Biscoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Biscoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 3.5 * 24 * 60 * 60; // Biscoin: 3.5 days
static const int64 nTargetSpacing = 2.5 * 60; // Biscoin: 2.5 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Biscoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Biscoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0xbb4003ff752a038b0a9165270715a8b9599303b53edab5c994325ccf9a0fd967");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "BisCoin Made with Samsung BagIdea";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 50 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("140990710fa689ad5023690c80f3a49c8f13f8d45b8c857fbcbc8bc4a8e4d3eb4b10f4d4604fa08dce601aaf0f470216fe1b51850b4acf21b179c45070ac7b03a9") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1533684215;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2084977206;

        if (fTestNet)
        {
            block.nTime    = 1533684163;
            block.nNonce   = 387469257;
        }
	...
	if (true && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x549a77eaab7a61e9ce18e6141fe34a35eba084cd8bf5722f3d87c12f3a4f84a3"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xf3, 0xc6, 0xb6, 0xd3 }; // Biscoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("BiscoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("BiscoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static BiscoinMiner(CWallet *pwallet)
{
    printf("BiscoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("biscoin-miner");
    ...
    try { loop {
        ...
        printf("Running BiscoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("BiscoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&BiscoinMiner, pwallet));
}
